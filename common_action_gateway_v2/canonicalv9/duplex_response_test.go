...
func TestGatewayResponseDeadlineIgnoresPrivateResponseSemantics(t *testing.T) {
	...
	want := origin.Add(26 * time.Millisecond)
	...
	for _, privateCase := range []string{
		...
		got := gatewayResponseDeadline(
			eligible, arrival, previous, 10*time.Millisecond, 5*time.Millisecond,
		)
		if !got.Equal(want) {
...
func TestGatewayResponseDeadlineUsesOnlyPublicRecurrence(t *testing.T) {
	...
	if want := origin.Add(60 * time.Millisecond); !got.Equal(want) {
...
func TestSyntheticT7ResponseDifferential(t *testing.T) {
	origin := time.Unix(0, 1_000_000_000)
	ordinaryTool := gatewayResponseDeadline(origin, origin, time.Time{}, 10*time.Millisecond, 5*time.Millisecond)
	agentAsTool := gatewayResponseDeadline(origin, origin, time.Time{}, 10*time.Millisecond, 5*time.Millisecond)
...
func TestSyntheticT9ResponseDifferential(t *testing.T) {
	origin := time.Unix(0, 1_000_000_000)
	earlyReady := gatewayResponseDeadline(origin, origin, time.Time{}, 10*time.Millisecond, 5*time.Millisecond)
	lateReadyWithinCutoff := gatewayResponseDeadline(origin, origin, time.Time{}, 10*time.Millisecond, 5*time.Millisecond)
