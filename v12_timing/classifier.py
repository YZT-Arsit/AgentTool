...
SKLEARN_RANDOM_STATE_MODULUS = 2**32


def coordinate_protocol_seed(
    profile: str, task: str, framework: str, observer: str
) -> int:
    material = "|".join(
...
def sklearn_random_state(seed: int) -> int:
    """Map a nonnegative protocol seed into scikit-learn's uint32 domain."""

    if seed < 0:
        raise ValueError("scikit-learn random_state source seed must be nonnegative")
    return int(seed % SKLEARN_RANDOM_STATE_MODULUS)
...
def frozen_models(seed: int) -> dict[str, ClassifierMixin]:
    ...
    return {
        "LOGISTIC_REGRESSION": make_pipeline(
            StandardScaler(),
            LogisticRegression(C=1.0, max_iter=2_000, random_state=random_state),
        ),
...
def deterministic_train_cv_folds(
    ...
    if folds < 2 or len(unique) < folds:
        raise ValueError(
            "TRAIN-only model selection requires at least one complete block per CV fold"
        )
    ordered = sorted(
        unique,
    ...
    if any(not fold for fold in output) or set().union(*map(set, output)) != set(
        unique
    ):
...
def _positive_scores(model: ClassifierMixin, values: np.ndarray) -> np.ndarray:
    ...
    if probabilities.shape != (len(values), 2):
        raise ValueError(
            "frozen binary classifier did not return two-class probabilities"
        )
    ...
    for model_index, name in enumerate(MODEL_NAMES):
        ...
        if np.any(~np.isfinite(out_of_fold)):
            raise AssertionError(
                "TRAIN CV did not score every TRAIN session exactly once"
            )
...
def fit_train_predict_eval(
    ...
    *,
    ...
    return FrozenTrainEvalPredictions(
        target[evaluation].copy(),
        groups[evaluation].copy(),
        output,
        len(train),
        len(evaluation),
    )
