...
ATTACK_DEVELOPMENT_PROFILE_ID = re.compile(
...
class TimingIndistinguishabilityProfile:
    ...
    def validate(self) -> "TimingIndistinguishabilityProfile":
        grammar = {
            ...
            EFFECTIVE_PUBLIC_CLOCK_V3: ATTACK_DEVELOPMENT_PROFILE_ID,
        ...
        if self.timing_semantic_revision == NOMINAL_COMMITMENT_V1:
            ...
            if self.admission_horizon_ms != 4500:
                raise ValueError("V12 timing-attack development freezes H4500")
            if self.round_period_ms not in PUBLIC_PERIOD_CANDIDATES_MS or self.pir_resolution_period_ms != 60:
                raise ValueError("V12 timing-attack development freezes Delta10/20/25 and PIR60")
        else:
            raise ValueError("unknown V12 timing semantic revision")
...
def timing_attack_candidate_profiles() -> tuple[TimingIndistinguishabilityProfile, ...]:
    return tuple(
        TimingIndistinguishabilityProfile(
