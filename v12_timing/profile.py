...
EFFECTIVE_PROFILE_V4R1_ID = re.compile(
    r"^V12-TIMING-INDIST-V4R1-H(?P<maximum>[1-9][0-9]*)-H(?P<horizon>[1-9][0-9]*)-P(?P<period>[1-9][0-9]*)-PIR(?P<pir>[1-9][0-9]*)$"
)
...
EFFECTIVE_PROFILE_V4R4_ID = re.compile(
    r"^V12-TIMING-INDIST-V4R4-H(?P<maximum>[1-9][0-9]*)-H(?P<horizon>[1-9][0-9]*)-P(?P<period>[1-9][0-9]*)-PIR(?P<pir>[1-9][0-9]*)$"
)
EFFECTIVE_PROFILE_V4R5_ID = re.compile(
    r"^V12-TIMING-INDIST-V4R5-H(?P<maximum>[1-9][0-9]*)-H(?P<horizon>[1-9][0-9]*)-P(?P<period>[1-9][0-9]*)-PIR(?P<pir>[1-9][0-9]*)$"
)
EFFECTIVE_PROFILE_V4R6_ID = re.compile(
    r"^V12-TIMING-INDIST-V4R6-H(?P<maximum>[1-9][0-9]*)-H(?P<horizon>[1-9][0-9]*)-P(?P<period>[1-9][0-9]*)-PIR(?P<pir>[1-9][0-9]*)$"
)
EFFECTIVE_PROFILE_V4R7_ID = re.compile(
    r"^V12-TIMING-INDIST-V4R7-H(?P<maximum>[1-9][0-9]*)-H(?P<horizon>[1-9][0-9]*)-P(?P<period>[1-9][0-9]*)-B(?P<bound>[1-9][0-9]*)-PIR(?P<pir>[1-9][0-9]*)$"
)
EFFECTIVE_PROFILE_V4R8_ID = re.compile(
    r"^V12-TIMING-INDIST-V4R8-H(?P<maximum>[1-9][0-9]*)-H(?P<horizon>[1-9][0-9]*)-P(?P<period>[1-9][0-9]*)-B(?P<bound>[1-9][0-9]*)-PIR(?P<pir>[1-9][0-9]*)$"
)
...
V4R7_PROVIDER_COMPLETION_BOUND_MS = 200
...
class TimingIndistinguishabilityProfile:
    ...
    response_initial_release_delay_ms: int = 0
    response_public_lag_ms: int = 0
    response_preparation_lead_ms: int = 0
    response_preparation_workers: int = 0
    ...
    def validate(self) -> TimingIndistinguishabilityProfile:
        grammar = {
            ...
            DUPLEX_PUBLIC_TIMING_VIRTUALIZATION_V4R4: EFFECTIVE_PROFILE_V4R4_ID,
            DUPLEX_PUBLIC_TIMING_VIRTUALIZATION_V4R5: EFFECTIVE_PROFILE_V4R5_ID,
            DUPLEX_PUBLIC_TIMING_VIRTUALIZATION_V4R6: EFFECTIVE_PROFILE_V4R6_ID,
            DUPLEX_PUBLIC_TIMING_VIRTUALIZATION_V4R7: EFFECTIVE_PROFILE_V4R7_ID,
            DUPLEX_PUBLIC_TIMING_VIRTUALIZATION_V4R8: EFFECTIVE_PROFILE_V4R8_ID,
        ...
        if self.timing_semantic_revision == NOMINAL_COMMITMENT_V1:
            ...
            if self.admission_horizon_ms != 4500:
                raise ValueError("V12 duplex V4R1 revision freezes H4500")
            if (
                self.round_period_ms not in (10, 20, 25)
                or self.pir_resolution_period_ms != 60
            ):
                raise ValueError("V12 duplex V4R1 freezes Delta10/20/25 and PIR60")
            if self.response_preparation_lead_ms != 25:
                raise ValueError("V12 duplex V4R1 response preparation lead changed")
            if self.pir_commitment_lead_ms != 5:
                raise ValueError("V12 duplex PIR commitment lead changed")
            if self.registry_answer_release_delay_ms != 50:
                raise ValueError("V12 duplex Registry answer release delay changed")
            ...
            if self.admission_horizon_ms != 4500:
                raise ValueError("V12 duplex V4R2 revision freezes H4500")
            if (
                self.round_period_ms not in (10, 20, 25)
                or self.pir_resolution_period_ms != 60
            ):
                raise ValueError("V12 duplex V4R2 freezes Delta10/20/25 and PIR60")
            if self.response_preparation_lead_ms != 50:
                raise ValueError("V12 duplex V4R2 response preparation lead changed")
            if self.response_preparation_workers != 6:
                raise ValueError("V12 duplex V4R2 response preparation workers changed")
            if self.pir_commitment_lead_ms != 5:
                raise ValueError("V12 duplex PIR commitment lead changed")
            if self.registry_answer_release_delay_ms != 50:
                raise ValueError("V12 duplex Registry answer release delay changed")
            ...
            if self.admission_horizon_ms != 4500:
                raise ValueError("V12 duplex V4R4 revision freezes H4500")
            if (
                self.round_period_ms not in (10, 20, 25)
                or self.pir_resolution_period_ms != 60
            ):
                raise ValueError("V12 duplex V4R4 freezes Delta10/20/25 and PIR60")
            if self.response_preparation_lead_ms != 50:
                raise ValueError("V12 duplex V4R4 response preparation lead changed")
            if self.response_preparation_workers != 6:
                raise ValueError("V12 duplex V4R4 response preparation workers changed")
            if self.pir_commitment_lead_ms != 20:
                raise ValueError("V12 duplex V4R4 PIR commitment lead changed")
            if self.registry_answer_release_delay_ms != 50:
                raise ValueError("V12 duplex Registry answer release delay changed")
            if self.registry_worker_lanes != 1 or self.registry_max_inflight != 100:
                raise ValueError("V12 duplex Registry bounded worker design changed")
            ...
            if self.admission_horizon_ms != 4500:
                raise ValueError("V12 duplex V4R5 revision freezes H4500")
            if (
                self.round_period_ms not in (10, 20, 25)
                or self.pir_resolution_period_ms != 60
            ):
                raise ValueError("V12 duplex V4R5 freezes Delta10/20/25 and PIR60")
            if self.response_initial_release_delay_ms != 50:
                raise ValueError(
                    "V12 duplex V4R5 initial response release delay changed"
                )
            if self.response_preparation_lead_ms != 20:
                raise ValueError(
                    "V12 duplex V4R5 steady response preparation lead changed"
                )
            if self.response_preparation_workers != 6:
                raise ValueError("V12 duplex V4R5 response preparation workers changed")
            if self.pir_commitment_lead_ms != 20:
                raise ValueError("V12 duplex V4R5 PIR commitment lead changed")
            if self.registry_answer_release_delay_ms != 50:
                raise ValueError("V12 duplex Registry answer release delay changed")
            if self.registry_worker_lanes != 1 or self.registry_max_inflight != 100:
                raise ValueError("V12 duplex Registry bounded worker design changed")
            ...
            if self.admission_horizon_ms != 4500:
                raise ValueError("V12 duplex V4R6 revision freezes H4500")
            if self.round_period_ms != 10 or self.pir_resolution_period_ms != 60:
                raise ValueError("V12 duplex V4R6 closure freezes P10/PIR60")
            if self.response_public_lag_ms != 30:
                raise ValueError("V12 duplex V4R6 public response lag changed")
            if self.response_preparation_lead_ms != 20:
                raise ValueError("V12 duplex V4R6 response preparation lead changed")
            if self.response_preparation_workers != 6:
                raise ValueError("V12 duplex V4R6 response preparation workers changed")
            if self.pir_commitment_lead_ms != 20:
                raise ValueError("V12 duplex V4R6 PIR commitment lead changed")
            if self.registry_answer_release_delay_ms != 50:
                raise ValueError("V12 duplex Registry answer release delay changed")
            if self.registry_worker_lanes != 1 or self.registry_max_inflight != 100:
                raise ValueError("V12 duplex Registry bounded worker design changed")
            ...
            if self.admission_horizon_ms != 4500:
                raise ValueError("V12 duplex V4R7 revision freezes H4500")
            if self.round_period_ms != 10 or self.pir_resolution_period_ms != 60:
                raise ValueError("V12 duplex V4R7 closure freezes P10/PIR60")
            if self.provider_completion_bound_ms != V4R7_PROVIDER_COMPLETION_BOUND_MS:
                raise ValueError("V12 duplex V4R7 provider completion bound changed")
            if int(match.group("bound")) != self.provider_completion_bound_ms:
                raise ValueError("V12 duplex V4R7 profile ID disagrees with public B")
            if self.response_public_lag_ms != 30:
                raise ValueError("V12 duplex V4R7 public response lag changed")
            if self.response_preparation_lead_ms != 20:
                raise ValueError("V12 duplex V4R7 response preparation lead changed")
            if self.response_preparation_workers != 6:
                raise ValueError("V12 duplex V4R7 response preparation workers changed")
            if self.pir_commitment_lead_ms != 20:
                raise ValueError("V12 duplex V4R7 PIR commitment lead changed")
            if self.registry_answer_release_delay_ms != 50:
                raise ValueError("V12 duplex Registry answer release delay changed")
            if self.registry_worker_lanes != 1 or self.registry_max_inflight != 100:
                raise ValueError("V12 duplex Registry bounded worker design changed")
            ...
            if self.admission_horizon_ms != 4500:
                raise ValueError("V12 duplex V4R8 revision freezes H4500")
            if self.round_period_ms != 10 or self.pir_resolution_period_ms != 60:
                raise ValueError("V12 duplex V4R8 closure freezes P10/PIR60")
            if self.provider_completion_bound_ms != V4R7_PROVIDER_COMPLETION_BOUND_MS:
                raise ValueError("V12 duplex V4R8 provider completion bound changed")
            if int(match.group("bound")) != self.provider_completion_bound_ms:
                raise ValueError("V12 duplex V4R8 profile ID disagrees with public B")
            if self.response_public_lag_ms != 30:
                raise ValueError("V12 duplex V4R8 public response lag changed")
            if self.response_preparation_lead_ms != 20:
                raise ValueError("V12 duplex V4R8 response preparation lead changed")
            if self.response_preparation_workers != 6:
                raise ValueError("V12 duplex V4R8 response preparation workers changed")
            if self.pir_commitment_lead_ms != 20:
                raise ValueError("V12 duplex V4R8 PIR commitment lead changed")
            if self.registry_answer_release_delay_ms != 50:
                raise ValueError("V12 duplex Registry answer release delay changed")
            if self.registry_worker_lanes != 1 or self.registry_max_inflight != 100:
                raise ValueError("V12 duplex Registry bounded worker design changed")
        ...
        if (
            ...
            and self.provider_completion_bound_ms != 50
        ) or self.terminal_rounds != 1:
    ...
    def go_plan_fields(self) -> dict[str, int | str]:
        ...
        return {
            ...
            "response_initial_release_delay_ms": self.response_initial_release_delay_ms,
            "response_public_lag_ms": self.response_public_lag_ms,
            "response_preparation_lead_ms": self.response_preparation_lead_ms,
            "response_preparation_workers": self.response_preparation_workers,
        }
...
def duplex_timing_candidate_profiles() -> tuple[TimingIndistinguishabilityProfile, ...]:
    return tuple(
        TimingIndistinguishabilityProfile(
            profile_id=f"V12-TIMING-INDIST-V4R5-H50-H4500-P{period}-PIR60",
            round_period_ms=period,
            pir_resolution_period_ms=60,
            admission_horizon_ms=4500,
            timing_semantic_revision=DUPLEX_PUBLIC_TIMING_VIRTUALIZATION_V4R5,
            response_initial_release_delay_ms=50,
...
def duplex_response_closure_p10_profile() -> TimingIndistinguishabilityProfile:
    """Public P10 profile for response startup/emission qualification only."""
    return TimingIndistinguishabilityProfile(
        profile_id="V12-TIMING-INDIST-V4R6-H50-H4500-P10-PIR60",
        round_period_ms=10,
        pir_resolution_period_ms=60,
        admission_horizon_ms=4500,
        timing_semantic_revision=DUPLEX_PUBLIC_TIMING_VIRTUALIZATION_V4R6,
...
def duplex_provider_bound_p10_profile() -> TimingIndistinguishabilityProfile:
    """Public V4R7 P10 profile with the pre-outcome selected provider bound."""
    return TimingIndistinguishabilityProfile(
        profile_id="V12-TIMING-INDIST-V4R7-H50-H4500-P10-B200-PIR60",
        round_period_ms=10,
        pir_resolution_period_ms=60,
        admission_horizon_ms=4500,
        provider_completion_bound_ms=V4R7_PROVIDER_COMPLETION_BOUND_MS,
        timing_semantic_revision=DUPLEX_PUBLIC_TIMING_VIRTUALIZATION_V4R7,
        response_public_lag_ms=30,
        response_preparation_lead_ms=20,
        response_preparation_workers=6,
        pir_commitment_lead_ms=20,
...
def duplex_response_anchor_p10_profile() -> TimingIndistinguishabilityProfile:
    """Public V4R8 P10 profile with an eligibility-anchored response clock."""
    return TimingIndistinguishabilityProfile(
        profile_id="V12-TIMING-INDIST-V4R8-H50-H4500-P10-B200-PIR60",
        round_period_ms=10,
        pir_resolution_period_ms=60,
        admission_horizon_ms=4500,
        provider_completion_bound_ms=V4R7_PROVIDER_COMPLETION_BOUND_MS,
        timing_semantic_revision=DUPLEX_PUBLIC_TIMING_VIRTUALIZATION_V4R8,
        response_public_lag_ms=30,
        response_preparation_lead_ms=20,
        response_preparation_workers=6,
        pir_commitment_lead_ms=20,
