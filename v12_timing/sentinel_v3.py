...
from .sentinel_resume import (
    FAILURE_EXACT_ALPHA,
    FAILURE_RATE_DIFFERENCE_MARGIN,
    OPERATIONAL_FAILURE_RATE_MARGIN,
    completion_channel,
...
SUPERSEDED_PROTOCOL_SHA = "3dde92221b274148f4926de4d4df07d8a6c64cd5"
PRIOR_ABORT_SHA = "63bdd948dc5364cf3bdfa85bcc5170f5c5d5712b"
V3_SEED_LABEL = "V12-P10-TIMING-DISTINGUISHABILITY-SENTINEL-V3-20260901"
WORKLOAD_BLOCK_OFFSET = 6000
PLANNED_BLOCKS = 315
...
def coordinate_seed(coordinate: SentinelCoordinate, purpose: str) -> int:
    return int.from_bytes(
        _digest(V3_SEED_LABEL, METHODOLOGY_BASE_SHA, coordinate.coordinate_id, purpose)[:8],
        "big",
    )


def build_v3_workload(
    task_id: str, framework: str, label: int, *, planned_block: int
) -> PrimaryTimingWorkload:
    if not 0 <= planned_block < PLANNED_BLOCKS:
        raise ValueError("V3 sentinel planned block is outside the frozen denominator")
...
def build_freeze_manifest(
    *,
        ...
        for planned_block in range(PLANNED_BLOCKS):
            ...
            for workload in members:
                ...
                if workload.identity in identities or workload.identity in excluded:
                    raise AssertionError("V3 sentinel identity is duplicated or excluded")
                identities[workload.identity] = row
            order = _class_order(coordinate, planned_block)
            pair_id = hashlib.sha256(
                f"{V3_SEED_LABEL}|{coordinate.coordinate_id}|B{planned_block}".encode()
            ).hexdigest()[:24]
            pair = {
                "pair_id": pair_id,
                "coordinate_id": coordinate.coordinate_id,
    ...
    manifest: dict[str, Any] = {
        "schema": "AgentTool.V12P10TimingSentinelV3Freeze/1",
        "phase": "V12-P10-TIMING-DISTINGUISHABILITY-SENTINEL-V3",
        "methodology_base_sha": METHODOLOGY_BASE_SHA,
        "protocol_base_sha": METHODOLOGY_BASE_SHA,
        "latest_development_evidence_sha": PRIOR_ABORT_SHA,
        "superseded_protocol_sha": SUPERSEDED_PROTOCOL_SHA,
        "prior_abort_sha": PRIOR_ABORT_SHA,
        "execution_source_commit": execution_source_commit,
        "frozen_before_first_protected_session": True,
        "seed_search": False,
        "identity_search": False,
        "seed_label": V3_SEED_LABEL,
        ...
        "completion_protocol": {
            ...
            "failure_channel_flag": (
                "absolute rate difference > 0.01 AND exact two-sided p < 0.05/8"
            ),
        ...
        "development_exclusions": {
            ...
            "all_prior_5040_resume_identities_excluded": True,
        },
...
def validate_freeze_manifest(
    ...
    if claimed != actual:
        raise ValueError("V3 sentinel frozen manifest payload hash drifted")
    if manifest["methodology_base_sha"] != METHODOLOGY_BASE_SHA:
        raise ValueError("V3 sentinel methodology base drifted")
    if manifest["superseded_protocol_sha"] != SUPERSEDED_PROTOCOL_SHA:
        raise ValueError("V3 sentinel superseded protocol lineage drifted")
    if manifest["profile"]["profile_id"] != P10_PROFILE_ID:
        raise ValueError("V3 sentinel P10 profile drifted")
    if int(manifest["physical_coordinate_count"]) != SENTINEL_PHYSICAL_COORDINATES:
        raise ValueError("V3 sentinel physical coordinate count drifted")
    if int(manifest["observer_comparison_count"]) != SENTINEL_OBSERVER_COMPARISONS:
        raise ValueError("V3 sentinel observer comparison count drifted")
    if int(manifest["total_physical_sessions"]) != TOTAL_SESSIONS:
        raise ValueError("V3 sentinel session denominator drifted")
    ...
    if len(identities) != TOTAL_SESSIONS or len(schedule) != TOTAL_SESSIONS:
        raise ValueError("V3 sentinel identity manifest is incomplete")
    scheduled = [str(row["identity"]) for row in schedule]
    if len(set(scheduled)) != TOTAL_SESSIONS or set(scheduled) != set(identities):
        raise ValueError("V3 sentinel schedule does not use every identity exactly once")
    if set(identities) & {str(value) for value in excluded_identities}:
        raise ValueError("V3 sentinel reuses an excluded identity")
    if tuple(manifest["feature_contract"]["RELAY_raw_widths"]) != (506, 505, 506, 505, 506):
        raise ValueError("V3 Relay raw feature widths drifted")
    if int(manifest["feature_contract"]["RELAY_feature_width"]) != 2589:
        raise ValueError("V3 Relay feature width drifted")
    ...
    if any(
        ...
        raise ValueError("V3 sentinel coordinate denominator drifted")
    pairs = manifest["pairs"]
    if len(pairs) != SENTINEL_PHYSICAL_COORDINATES * PLANNED_BLOCKS:
        raise ValueError("V3 sentinel pair inventory is incomplete")
    for pair in pairs:
        rows = [identities[identity] for identity in pair["member_identities_in_execution_order"]]
        if sorted(int(row["label"]) for row in rows) != [0, 1]:
            raise ValueError("V3 sentinel pair lost one protected class")
        if rows[0]["public_profile_signature"] != rows[1]["public_profile_signature"]:
            raise ValueError("V3 sentinel pair public signature mismatch")
