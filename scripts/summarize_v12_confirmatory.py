...
DEFAULT_RESULTS = ROOT / "results_v12_confirmatory"
DEFAULT_PLAN = ROOT / "V12_EXECUTION_PLAN.json"
DEFAULT_RULES = ROOT / "V12_FINAL_DECISION_RULES.json"
DEFAULT_TRAJECTORIES = ROOT / "V12_CAUSAL_TRAJECTORY_HOLDOUT_FREEZE.json"
...
def summarize_campaign(
    ...
    *,
    ...
    execution_integrity = all(
        (
            len(ledger) == rules["execution_integrity"]["expected_units"] == 134,
            len(set(ledger_ids)) == len(ledger_ids),
            ledger_ids == planned_ids,
            [row.get("global_execution_index") for row in ledger] == list(range(1, 135)),
            ledger_chain_valid,
    ...
    summary = {
        "schema": "AgentTool.V12ConfirmatorySummary/1",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Frozen pure V12 confirmatory summarizer")
    parser.add_argument("--results-root", type=Path, default=DEFAULT_RESULTS)
    args = parser.parse_args()
    output = args.results_root / "V12_CONFIRMATORY_SUMMARY.json"
