...
def fresh_structural_specs():
    fresh = []
    for pair_index, (name, arm_a, arm_b) in enumerate(structural_specs(), 1):
        arms = []
        for arm_name, (workflow, cases) in zip(("A", "B"), (arm_a, arm_b), strict=True):
            values = [
                replace(
                    case,
                    case_id=f"DEV-V12_1-REQUAL-STR-P{pair_index:02d}-{arm_name}-{index:02d}",
                    operation_id=f"opV121RQSP{pair_index:02d}{arm_name}{index:02d}",
                )
                for index, case in enumerate(cases, 1)
            ]
            arms.append((workflow, values))
        fresh.append((name, arms[0], arms[1]))
    return fresh
...
def capability_preflight(root: Path, runner: Path) -> dict[str, Any]:
    case_oa = strict_cases(1, "DEV-V12_1-REQUAL-PREFLIGHT-OPENAI")[0]
    case_ms = strict_cases(1, "DEV-V12_1-REQUAL-PREFLIGHT-MICROSOFT", "Microsoft Agent Framework")[0]
...
def final_reliability(root: Path, runner: Path) -> list[dict[str, Any]]:
    ...
    for count, repetitions in {10: 100, 20: 50, 30: 50, 50: 50}.items():
        for iteration in range(repetitions):
            value = execute(root / f"causal_{count}" / f"{iteration:03d}", runner, "OpenAI Agents SDK", "DYNAMIC_SEQUENCE", strict_cases(count, f"DEV-V12_1-REQUAL-REL-C{count}-{iteration:03d}"))
    ...
    for kind in ("TOOL_TO_AGENT_AS_TOOL", "AGENT_AS_TOOL_TO_TOOL", "TOOL_TO_HANDOFF", "MICROSOFT_TOOL_TO_AGENT_AS_TOOL"):
        for iteration in range(25):
            framework, workflow, cases = mixed_spec(kind, f"DEV-V12_1-REQUAL-REL-{kind}-{iteration:03d}")
    ...
    for iteration in range(50):
        framework, workflow, cases = mixed_spec("STRUCTURED_TOOL_TO_AGENT_AS_TOOL", f"DEV-V12_1-REQUAL-REL-STRUCT-{iteration:03d}")
...
def mixed_qualification(root: Path, runner: Path) -> list[dict[str, Any]]:
    ...
    for kind in MIXED_FAMILIES:
        for iteration in range(30):
            framework, workflow, cases = mixed_spec(kind, f"DEV-V12_1-REQUAL-MIX-{kind}-{iteration:03d}")
...
def regressions(root: Path, runner: Path) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for index, kind in enumerate((*MIXED_FAMILIES, "STRICT_CAUSAL_10", "MICROSOFT_CAUSAL_3")):
        if kind == "STRICT_CAUSAL_10":
            framework, workflow, cases = "OpenAI Agents SDK", "DYNAMIC_SEQUENCE", strict_cases(10, "DEV-V12_1-REQUAL-SEM-OA10")
        elif kind == "MICROSOFT_CAUSAL_3":
            framework, workflow, cases = "Microsoft Agent Framework", "DYNAMIC_SEQUENCE", strict_cases(3, "DEV-V12_1-REQUAL-SEM-MS3", "Microsoft Agent Framework")
        else:
            framework, workflow, cases = mixed_spec(kind, f"DEV-V12_1-REQUAL-SEM-{kind}")
    ...
    for name, (workflow_a, cases_a), (workflow_b, cases_b) in fresh_structural_specs():
        a = execute(
...
def main() -> None:
    ...
    write_json(root / "result.json", {
        "schema": "AgentTool.V12_1ProfileRequalification/1",
