...
OUTPUT_ROOT = ROOT / "results_v12_confirmatory"
PLAN_PATH = ROOT / "V12_EXECUTION_PLAN.json"
BASELINE_PATH = ROOT / "V12_FINAL_CONFIRMATORY_FREEZE.json"
DRIVER_FREEZE_PATH = ROOT / "V12_FINAL_CONFIRMATORY_FREEZE.json"
HARDENED_FREEZE_PATH = ROOT / "V12_FINAL_CONFIRMATORY_FREEZE.json"
SIMPLEPIR_BRIDGE_FREEZE_PATH = ROOT / "V12_SIMPLEPIR_RUNTIME_CLOSURE.json"
FINAL_DECISION_RULES_PATH = ROOT / "V12_FINAL_DECISION_RULES.json"
ARTIFACT_MANIFEST_PATH = ROOT / "V12_EXECUTION_ARTIFACT_MANIFEST.json"
AUTHORIZED_V12_FREEZE_FIELD = "authorized_v12_artifact_manifest_sha256"
SEMANTIC_MANIFESTS = (
    ("S1", ROOT / "V12_SOURCE_SEMANTIC_HOLDOUT_FREEZE.json"),
    ("S2", ROOT / "V12_COMPOSITION_SEMANTIC_HOLDOUT_FREEZE.json"),
    ("S4", ROOT / "V12_EFFECT_CONTRACT_HOLDOUT_FREEZE.json"),
)
TRAJECTORY_MANIFEST = ROOT / "V12_CAUSAL_TRAJECTORY_HOLDOUT_FREEZE.json"
STRUCTURAL_MANIFEST = ROOT / "V12_STRUCTURAL_SIZE_HOLDOUT_FREEZE.json"
ORDER_PATH = ROOT / "V12_EXECUTION_ORDER.json"
...
def build_execution_plan() -> dict[str, Any]:
    ...
    if (len(units), native, canonical) != (134, 53, 81):
        raise HarnessIntegrityFailure("frozen execution-plan counts are not 134/53/81")
    return {
        "schema": "AgentTool.V12ExecutionPlan/1",
        "phase_order": [
            "semantic_case_order",
...
def verify_execution_artifact_manifest(
    ...
    checks: dict[str, bool] = {
        "manifest_status": manifest.get("status") == "FROZEN_FOR_V12_EXECUTION",
        "historical_commit_recorded": bool(manifest.get("historical_source_commit")),
    }
    ...
    checks["frozen_files"] = verify_frozen_files(manifest, root=root)
...
def verify_frozen_files(manifest: dict[str, Any], *, root: Path = ROOT) -> bool:
    """Verify exact bytes for every execution artifact, including the public profile."""
...
def preflight(
    ...
    *,
    ...
    if authorization.get("phase") != "V12" or authorization.get("approved") is not True:
        raise HarnessIntegrityFailure("independent V12 authorization is absent")
    authorized_freeze = str(authorization.get(AUTHORIZED_V12_FREEZE_FIELD, ""))
    if authorized_freeze != sha256(ARTIFACT_MANIFEST_PATH):
        raise HarnessIntegrityFailure("authorization does not bind the frozen V12 artifact manifest")
    if output_root.exists():
        raise HarnessIntegrityFailure("results_v12_confirmatory already exists")
    ...
    from scripts.run_v12_profile_requalification import capability_preflight
    from v12_development.resources import snapshot

    resource_before = snapshot()
    with tempfile.TemporaryDirectory(prefix="agenttool-v12-capability-") as temporary:
        capability = capability_preflight(Path(temporary) / "capability", runner_path)
    resource_after = snapshot()
    resource_ok = all(
        (
            resource_after["open_fd_count"] <= resource_before["open_fd_count"] + 8,
            resource_after["zombie_children"] == 0,
            resource_after["simplepir_processes"] == 0,
            resource_after["canonical_runner_processes"] == 0,
        )
    )
    if capability.get("status") != "PASS" or not resource_ok:
        raise HarnessIntegrityFailure("development capability/resource preflight failed before permit")
    return {
        "authorization": authorization,
        "authorized_commit": str(load_json(ARTIFACT_MANIFEST_PATH).get("historical_source_commit", "UNCOMMITTED_FREEZE")),
        "authorized_artifact_manifest_sha256": authorized_freeze,
        "artifact_manifest_verification": artifacts,
        "capability_preflight": capability,
        "resource_preflight": {"before": resource_before, "after": resource_after, "passed": resource_ok},
    }
...
def _approved_permit_after_preflight(preflight_result: dict[str, Any]) -> ExecutionPermit:
    ...
    return ExecutionPermit("V12", True, capability_preflight_passed=True)
...
def execute_unit(
    ...
    if unit["phase"] == "1_SEMANTIC":
        ...
        if role == "NATIVE":
            ...
            value = {"record": _jsonable(record), "projection": record.projection()}
            native_path = unit_dir.parent / f"V12-U{unit['global_execution_index'] - 1:03d}" / "unit_result.json"
...
def write_campaign_completion(
    ...
    *,
    ...
    if len(ledger_records) != 134:
        raise HarnessIntegrityFailure("campaign completion requires exactly 134 ledger records")
    ...
    write_json_exclusive(
        ...
        {
            "schema": "AgentTool.V12CampaignCompletion/1",
            "expected_unit_count": 134,
...
def run_campaign(authorization_path: Path, runner_path: Path) -> int:
    ...
    content = load_frozen_content()
    campaign = {
        "schema": "AgentTool.V12CampaignManifest/1",
    ...
    from scripts.summarize_v12_confirmatory import summarize_campaign

    summary_path = OUTPUT_ROOT / "V12_CONFIRMATORY_SUMMARY.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Frozen V12 one-shot confirmatory driver")
