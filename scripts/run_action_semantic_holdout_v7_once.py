...
from action_privacy_v6.adapters import (
    DeterministicLocalProvider,
    FrameworkActionAdapter,
    NativeAction,
    execute_mediated,
    execute_native,
)
...
FREEZE = ROOT / "ACTION_SEMANTIC_HOLDOUT_V7_FREEZE.json"
FREEZE_HASH = ROOT / "ACTION_SEMANTIC_HOLDOUT_V7_FREEZE_SHA256.txt"
OUTPUT = ROOT / "ACTION_SEMANTIC_HOLDOUT_V7.csv"


def main() -> None:
    if OUTPUT.exists():
        raise FileExistsError(f"one-shot result already exists: {OUTPUT}")
    expected_hash = FREEZE_HASH.read_text(encoding="utf-8").strip()
    if hashlib.sha256(FREEZE.read_bytes()).hexdigest() != expected_hash:
        raise RuntimeError("V7 holdout freeze hash mismatch")
    ...
    for ordinal, case in enumerate(manifest["cases"]):
        ...
        descriptor_key = bytes((ordinal + i + 17) % 256 for i in range(32))
        gateway_key = bytes((ordinal + i + 149) % 256 for i in range(32))
        descriptor = AgentDescriptorV6(ordinal, (case["action_name"],), "v7-holdout", 1,
        ...
        mediated = execute_mediated(action, adapter, dispatch, session_id=f"v7-holdout-{ordinal}")
        ...
        rows.append({
            ...
            "native_reference_pass": native_dict == expected,
            "mediated_pass": mediated_dict == expected,
    ...
    summary = {
        "cases": len(rows),
        "passed": sum(bool(row["semantic_pass"]) for row in rows),
        "status": "EXECUTED_ONCE_NO_TUNING",
        "scope": manifest["scope"],
    }
    (ROOT / "results_v7").mkdir(exist_ok=True)
    (ROOT / "results_v7/action_semantic_holdout_summary.json").write_text(
